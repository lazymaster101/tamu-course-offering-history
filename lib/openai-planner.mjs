...
function extractCourseCodesFromText(text) {
  return [...new Set(
    [...String(text ?? "").toUpperCase().matchAll(/\b([A-Z]{3,5})\s*(\d{3}[A-Z]?)\b/gu)]
...
function buildPlannerContextCourses(plannerState, question) {
  const courseMap = new Map();

  function upsertCourse(code, title, priority = 5) {
    const normalizedCode = normalizeCourseCodeToken(code);
    if (!normalizedCode) {
      return;
    }
    ...
    if (!existing) {
      courseMap.set(normalizedCode, {
        code: normalizedCode,
        title: String(title ?? normalizedCode).trim() || normalizedCode,
        contextPriority: priority
      });
      return;
    }

    if (priority < existing.contextPriority) {
      existing.contextPriority = priority;
    }

    if ((!existing.title || existing.title === existing.code) && title) {
      existing.title = String(title).trim() || existing.code;
    }
  }

  for (const course of plannerState?.plannedCourses ?? []) {
    upsertCourse(course?.code, course?.title, 0);
  }

  for (const section of plannerState?.scheduleRecommendation?.sections ?? []) {
    upsertCourse(section?.courseCode, section?.courseTitle, 1);
  }

  for (const source of plannerState?.shortlistSources ?? []) {
    upsertCourse(getShortlistCourseCode(source), source?.label, 1);
  }

  for (const code of extractCourseCodesFromText(question)) {
    upsertCourse(code, code, 1);
  }

  for (const course of plannerState?.eligibleRequiredCourses ?? []) {
    upsertCourse(course?.code, course?.title, 2);
  }

  for (const course of plannerState?.trackedElectiveSuggestions ?? []) {
    upsertCourse(course?.code, course?.title, 3);
  }

  for (const course of plannerState?.fastTrackOptions ?? []) {
    if (course?.undergraduateCode) {
      upsertCourse(course.undergraduateCode, course?.title, 4);
    }
    if (course?.graduateCode) {
      upsertCourse(course.graduateCode, course?.title, 4);
    }
  }

  return [...courseMap.values()].sort((left, right) => {
    if (left.contextPriority !== right.contextPriority) {
      return left.contextPriority - right.contextPriority;
    }
...
function buildRelevantSyllabusGroups(plannerState, question) {
  const relevantCodes = new Set(
    [
      ...(plannerState?.plannedCourses ?? []).map((course) => course?.code),
      ...extractCourseCodesFromText(question),
      ...(
...
async function buildPlannerLiveContext(plannerState, question) {
  ...
  const contextCourses = buildPlannerContextCourses(plannerState, question);

  if (!contextCourses.length) {
    return {
...
async function buildSyllabusComparisonContext(plannerState, question, requestOrigin) {
  ...
  const groups = buildRelevantSyllabusGroups(plannerState, question).slice(0, 6);
  if (!groups.length) {
    return [];
  }
...
function buildPlannerInstructions() {
  return [
    ...
    "If the planner state includes upcomingOfferings, treat it as the live next-term section inventory gathered from TAMU public class search for the planner-visible courses discussed in the current state and question, not just the current semester plan.",
    ...
    "If syllabusComparisons are present in the planner state, treat them as grounded syllabus evidence and use them when recommending instructors or critiquing the schedule. Historical syllabus-bearing terms may be used when the newest term has live section rows but no exposed syllabus links yet.",
    ...
    "If upcomingOfferings exists, mention when a course has no offered sections in the target term, and distinguish that from courses that are offered but only have historical syllabus evidence.",
...
export async function chatWithDegreePlanner({
  ...
  const plannerStateWithLiveContext = {
    ...
    contextCourses: liveContext.contextCourses
  };
