...
export async function buildScheduleRecommendation({
  ...
  for (const course of parsedCourses) {
    const sections = await getCourseSections(course.subject, course.courseNumber, targetTerm.termCode);
  ...
  const schedules = buildScheduleOptions(courseCandidates).map((candidate, index) => ({
    id: `schedule-${index + 1}`,
