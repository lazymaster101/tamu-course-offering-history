...
function buildSyllabusSourcesForCourse(course, targetTerm, sections) {
...
export async function buildScheduleRecommendation({
  ...
  let termSectionsFetchFailure = null;

  for (const course of parsedCourses) {
    let sections = [];

    if (termSectionsFetchFailure) {
      unavailableCourses.push({
        code: course.code,
        title: course.title,
        reason: `Live section data for ${targetTerm.termDescription} is temporarily unavailable: ${termSectionsFetchFailure.message}`
      });
      continue;
    }
    ...
    try {
      ...
      termSectionsFetchFailure = error;
      unavailableCourses.push({
        code: course.code,
        title: course.title,
...
export async function buildUpcomingOfferingContext({
  ...
  for (const course of parsedCourses) {
    ...
    offerings.push({
      ...
      syllabusSources: buildSyllabusSourcesForCourse(course, targetTerm, scoredSections)
    });
  }
